...
def q_promo(dbClient, segment):
  ...
  try:
    ...
    # Promos activas del segmento, una fila por edificio/tipo de piso/tipo de plaza.
    # La elección de la mejor promo la hace la web (mínimo de los pct).
    sql = '''
      SELECT DISTINCT r."Building_id" AS "building", r."Segment_id", rft."Code" AS "flat_type", rpt."Code" AS "place_type",
        ROUND(p."Value_rent_pct", 1) AS "Value_rent_pct", ROUND(p."Value_fee_pct", 1) AS "Value_fee_pct",
      ...
      FROM "Billing"."Promotion" p
        INNER JOIN "Resource"."Resource" r
          ...
          -- Limitación: el edificio debe tener recursos del régimen que anuncia la promo.
          AND (
